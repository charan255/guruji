...
import { ArrowLeft, Scale, Check, X, Disc } from 'lucide-react';

const CareerComparison = ({ onBack }) => {
    const [careerA, setCareerA] = useState(CAREERS[0]);
    const [careerB, setCareerB] = useState(CAREERS[1]);

    const handleSelectA = (e) => {
        const selected = CAREERS.find(c => c.id === e.target.value);
        setCareerA(selected);
    };
    ...
    const attributes = [
        { label: 'Work Type', key: 'type' },
        { label: 'Learning Curve', key: 'learning' },
        ...
        <div className="container animate-fade-in" style={{ paddingBottom: '4rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: '2rem', alignItems: 'center', marginBottom: '3rem' }}>
                <div className="glass-card" style={{ padding: '1rem' }}>
                    ...
                    >
                        {CAREERS.map(c => <option key={c.id} value={c.id} style={{ background: '#0f172a' }}>{c.title}</option>)}
                    </select>
                </div>
                ...
                {attributes.map((attr, idx) => (
                    <div key={idx} style={{
                        ...
                        <div style={{ textAlign: 'center' }}>
                            {careerA.comparison ? careerA.comparison[attr.key] : '-'}
                        </div>
                        <div style={{ textAlign: 'center' }}>
                            {careerB.comparison ? careerB.comparison[attr.key] : '-'}
                        </div>
                    </div>
                ))}
                ...
                <div style={{
                    ...
                    <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'center' }}>
                        {careerA.skills?.slice(0, 3).map(s => (
                            <span key={s.name} className="badge" style={{ background: 'rgba(139, 92, 246, 0.1)', color: '#c4b5fd' }}>{s.name}</span>
                        ))}
                    </div>
                    <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'center' }}>
                        {careerB.skills?.slice(0, 3).map(s => (
                            <span key={s.name} className="badge" style={{ background: 'rgba(244, 63, 94, 0.1)', color: '#fda4af' }}>{s.name}</span>
                        ))}
                    </div>
                </div>
            </div>
            ...
            <div style={{ marginTop: '3rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                <div className="glass-card" style={{ borderTop: '4px solid var(--primary)' }}>
                    ...
                    <p style={{ opacity: 0.8, lineHeight: '1.6' }}>{careerA.comparison?.focus}</p>
                </div>
                <div className="glass-card" style={{ borderTop: '4px solid var(--accent)' }}>
                    ...
                    <p style={{ opacity: 0.8, lineHeight: '1.6' }}>{careerB.comparison?.focus}</p>
                </div>
            </div>
